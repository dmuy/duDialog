			...
			var _ = this, _callbacks = _.config.callbacks, wrapper, header, content, footer,
				...
				maxSelectCheck = function () {
					...
					if (checked.length === _.config.maxSelect) {
						...
						if(_callbacks && _callbacks.maxReached) _callbacks.maxReached.call(_, _.config.maxSelect)
					} else {
						content.querySelectorAll('.dlg-select-checkbox:not(:checked)').forEach(function (cb) {
							...
							if (e.target.matches('.ok-action')) {
								if (_.config.selection && _.config.multiple) {
									...
									if (checkedVals.length >= _.config.minSelect) {
										...
										if(_callbacks && _callbacks.minRequired) _callbacks.minRequired.call(_, _.config.minSelect)
									}
								} else {
